  ...
  <div class="course-container course-lectures-view-page">
    <div class="d-flex">
      ...
      <div class="right-part">
        <h1>Lecture: {{ $first_lecture->lecture_title }}</h1>
        <video controls>
          <source src="{{ asset($first_lecture->lecture_video) }}" type="video/mp4">
          Your browser does not support the video tag.
        </video>
        <a href="{{ $first_lecture->lecture_youtube }}" target="_blank" class="button red ui"><i class="fab fa-fw fa-youtube"></i> YouTube</a>
        <a download="{{ strtolower( str_replace(' ', '_', $first_lecture->lecture_title)) }}" href="{{ asset($first_lecture->lecture_file) }}" class="button ui inverted green"><i class="fas fa-fw fa-download"></i> Download Lecture</a>
        <a href="{{ route('admin.courses.lectures.update', [$course->course_id, $first_lecture->lecture_id]) }}" class="button ui inverted olive"><i class="fas fa-fw fa-wrench"></i> Update Lecture</a>
      </div>
    </div>
    <h1>Lecture Description</h1>
    <div class="article-content">
      {!! $first_lecture->lecture_content !!}
    </div>
  </div>
@endsection
